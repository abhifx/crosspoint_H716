...
void FreeInkDisplay::clearScreen(uint8_t color) const {
  if (displayWidthBytes == displayWidth) {
    // 8-bit framebuffer clear (0=White, 15=Black)
    memset(frameBuffer, color == 0x00 ? 15 : 0, bufferSize);
    return;
  }
...
void FreeInkDisplay::blitImage(const uint8_t* imageData, uint16_t x, uint16_t y, uint16_t w, uint16_t h,
  ...
  if ((x & 7) == 0) {
    ...
    for (uint16_t row = 0; row < h; row++) {
      ...
      for (uint16_t col = 0; col < imageWidthBytes; col++) {
        ...
        if (displayWidthBytes == displayWidth) {
          // Native 8-bit linear expansion (1bpp source to 8bpp destination)
          for (int bit = 0; bit < 8; bit++) {
            const int gx = (xByte + col) * 8 + bit;
            if (gx >= displayWidth) break;
            const bool white = (srcByte >> (7 - bit)) & 1;
            if (transparent) {
              if (!white) frameBuffer[static_cast<size_t>(destY) * displayWidth + gx] = 15;
            } else {
              frameBuffer[static_cast<size_t>(destY) * displayWidth + gx] = white ? 0 : 15;
            }
          }
          continue;
        }
  ...
  for (uint16_t row = 0; row < h; row++) {
    ...
    for (uint16_t col = 0; col < w; col++) {
      ...
      if (displayWidthBytes == displayWidth) {
        // Native 8-bit linear expansion
        if (transparent) {
          if (!white) frameBuffer[static_cast<size_t>(destY) * displayWidth + destX] = 15;
        } else {
          frameBuffer[static_cast<size_t>(destY) * displayWidth + destX] = white ? 0 : 15;
        }
        continue;
      }
...
bool FreeInkDisplay::reallocBuffers() {
  ...
  frameBuffer = frameBuffer0;
  if (displayWidthBytes == displayWidth) {
    memset(frameBuffer0, 0x00, bufferSize); // White (0)
  } else {
  ...
  frameBufferActive = frameBuffer1;
  if (displayWidthBytes == displayWidth) {
    memset(frameBuffer1, 0x00, bufferSize);
  } else {
...
void FreeInkDisplay::returnBuildStorage() {
  ...
  _buildLent = false;
  frameBuffer = frameBuffer0;
  if (frameBuffer0 != nullptr) {
    if (displayWidthBytes == displayWidth) {
      memset(frameBuffer0, 0x00, bufferSize);
    } else {
      memset(frameBuffer0, 0xFF, bufferSize);
    }
  }
...
bool FreeInkDisplay::reallocSecondaryBuffer() {
  ...
  *slot = allocFrameBufferStorage();
  ...
  if (frameBuffer) {
    ...
    if (displayWidthBytes == displayWidth) {
      memset(frameBufferActive, 0x00, bufferSize);
    } else {
