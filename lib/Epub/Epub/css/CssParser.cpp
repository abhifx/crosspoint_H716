...
bool tryParseNumber(std::string_view s, T& out) {
  ...
  // GCC 8 does not support from_chars for float. Use strtof for float.
  if constexpr (std::is_floating_point_v<T>) {
    std::string temp(s);
    char* end;
    out = strtof(temp.c_str(), &end);
...
size_t CssParser::SvHash::operator()(const std::string& s) const noexcept {
  size_t h = FNV_OFFSET_BASIS;
  for (char c : s) h = fnv1aMix(h, asciiToLower(c));
  return h;
}
...
void CssParser::processRuleBlockWithStyle(std::string_view selectorGroup, const CssStyle& style) {
  ...
  forEachDelimitedToken(
      ...
      [&](std::string_view sel) {
        ...
        // Convert to string for lookup since GCC 8 doesn't support transparent hashing for unordered_map
...
CssStyle CssParser::resolveStyle(std::string_view tagName, std::string_view classAttr) const {
  ...
  forEachDelimitedToken(classAttr, isCssWhitespace, [&](std::string_view cls) {
    if (auto it = rulesBySelector_.find(std::string(".") + std::string(cls)); it != rulesBySelector_.end()) {
      result.applyOver(it->second);
    }
  });
  ...
  forEachDelimitedToken(classAttr, isCssWhitespace, [&](std::string_view cls) {
    if (auto it = rulesBySelector_.find(std::string(tagName) + "." + std::string(cls)); it != rulesBySelector_.end()) {
      result.applyOver(it->second);
    }
  });
