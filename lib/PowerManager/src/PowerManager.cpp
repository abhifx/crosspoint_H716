...
void PowerManager::powerDownRailsForSleep() {
  ...
  holdRail(b.display.powerEnable, LOW);
  ...
  holdRail(b.sd.powerEnable, b.sd.powerActiveHigh ? LOW : HIGH);
  holdRail(b.touch.powerEnable, b.touch.powerEnableActiveHigh ? LOW : HIGH);
  // The mic enable also carries a polarity flag; OFF is the inactive level.
  holdRail(b.mic.enable, b.mic.enableActiveHigh ? LOW : HIGH);

  // Maintain power latches so the board stays on during deep sleep.
  holdRail(b.power.latch0, HIGH);
  holdRail(b.power.latch1, HIGH);
}
