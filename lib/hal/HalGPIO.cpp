...
#if FREEINK_DEVICE_LILYGO_H716
  // H716 Standard: uses GPIO 43 for USB detection.
...
HalGPIO::WakeupReason HalGPIO::getWakeupReason() const {
  ...
  // For H716, treat cold boot as PowerButton if not USB, or AfterFlash if USB.
  // This avoids the AfterUSBPower auto-sleep loop.
  if (wakeupCause == ESP_SLEEP_WAKEUP_UNDEFINED) {
    if (resetReason == ESP_RST_POWERON) {
      return WakeupReason::PowerButton;
    }
    if (resetReason == ESP_RST_UNKNOWN && usbConnected) {
