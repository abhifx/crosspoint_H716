...
def main():
    ...
    # 1. Save current src and lib back to its firmware folder (safety)
    if current_fw != "unknown":
        ...
        fw_lib_dest = firmware_root / current_fw / "lib"
        active_lib = project_root / "lib"
        if active_lib.exists():
            if fw_lib_dest.exists():
                shutil.rmtree(fw_lib_dest)
            shutil.copytree(active_lib, fw_lib_dest)
            print(f"Saved current '{current_fw}' to firmware/{current_fw}/lib")

    # 2. Clear active src and lib
    if active_src.exists():
        shutil.rmtree(active_src)
    active_lib = project_root / "lib"
    if active_lib.exists():
        shutil.rmtree(active_lib)

    # 3. Copy new firmware to src and lib
    new_fw_src = firmware_root / target_fw / "src"
    shutil.copytree(new_fw_src, active_src)

    new_fw_lib = firmware_root / target_fw / "lib"
    if new_fw_lib.exists():
        shutil.copytree(new_fw_lib, active_lib)
